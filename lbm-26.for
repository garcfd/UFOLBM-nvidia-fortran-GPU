...
CCC   integer, parameter :: ndim=3, nvec=15, nrow=5, nnrm=66
      ...
      real neq,f1,cs2,piab,Qixx,Qiyy,Qizz,Qixy,Qixz,Qiyz
...
C-----from sangtani thesis (p55)

C-----magnitude of non-equilibrium stress tensor sqrt(PIab*PIab)
      piab = sqrt( piNeq_xx**2.0 + piNeq_yy**2.0 + piNeq_zz**2.0
     &     +  2.0*(piNeq_xy**2.0 + piNeq_xz**2.0 + piNeq_yz**2.0) )

C-----from moritz notes twitter
      nut(i,j,k) = (sqrt(nuLB*nuLB + 18.0*smag*smag*piab) - nuLB)/6.0

c      if ((i.eq.280).and.(j.eq.80).and.(k.eq.3)) then
c        write(6,*) "nut=",nut(i,j,k)
c      endif
...
C       Replace fin by regularized counterpart
        fout(i,j,k,n) = feq(i,j,k,n) + (1.0-omega)*f1 ! F1
C       fout(i,j,k,n) = omega*feq(i,j,k,n) + (1.0-omega)*fin(i,j,k,n)
      enddo
